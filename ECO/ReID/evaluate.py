...
def evaluate(qf, ql, qc, gf, gl, gc):
    ...
    good_index = np.setdiff1d(query_index, camera_index, assume_unique=True) # same people from different cameras
    junk_index1 = np.argwhere(gl == -1)
    junk_index2 = np.intersect1d(query_index, camera_index) # same people from same camera
...
def compute_mAP(index, qc, good_index, junk_index):
    ...
    #print("r_g",np.shape(rows_good))
    rows_good = rows_good.flatten()

    cmc[rows_good[0]:] = 1
    #print("cmc_shape",np.shape(cmc))
