...
% 此函数计算 det1（单个）与det2（单个或者多个）的overlap 
idx2_list = idx2_list(:)';
n = length(idx2_list);

x1_1 = bboxes1.x(idx1); % 左上角点x
x2_1 = bboxes1.x(idx1) + bboxes1.w(idx1) - 1; % 右下角点x
y1_1 = bboxes1.y(idx1); % 左上角点y
y2_1 = bboxes1.y(idx1) + bboxes1.h(idx1) - 1; % 右下角点y
...
index_overlapped = find((w_inter > 0) .* (h_inter > 0)); % 直接计算有重叠面积的部分
ov = zeros(1, n);
occ1 = zeros(1, n);
occ2 = zeros(1, n);
