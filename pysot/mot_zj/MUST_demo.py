...
dataset_dir = '.\\data\\MOT_ZJ'
seq_type = 'train'
# set the path of config parameters and 
config_path = ".\\mot_zj\\MUST_config_file\\alex_config.yaml"
model_params = ".\\mot_zj\\MUST_config_file\\alex_model.pth"
...
if not os.path.exists(".\\results"):
    os.makedirs('.\\results')
if os.path.exists(".\\img_traj"):
    shutil.rmtree(".\\img_traj")
...
for seq_name in seq_names:
    ...
    for nn, im_path in enumerate(images):
        ...
        if frame % 100 == 0:
            ...
            np.savetxt('./results/'+filename, results_bboxes, fmt='%d,%d,%.1f,%.1f,%.1f,%.1f')
        ############################################
        #        ***crop tracklet image***         #
        ############################################
        ...
        for tracker in trackers:
            if tracker.track_state == cfg.STATE.START or tracker.track_state == cfg.STATE.TRACKED or tracker.track_state == cfg.STATE.ACTIVATED:
                ...
                traj_path = os.path.join(".\\img_traj", seq_name, str(tracker.id_num))
